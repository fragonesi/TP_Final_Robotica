"""Diagnóstico visual de la odometría (fuera de ROS).

Lee el CSV de `odom_delta_node` e imprime un resumen (duración, traslación y
rotación acumuladas, estadísticas de los deltas) más un panel de 4 gráficos:
trayectoria cruda (x,y), orientación en el tiempo, y los deltas de traslación y
rotación por paso. Sirve para chequear de un vistazo que la odometría tenga sentido
(forma de laberinto, sin saltos raros) antes de meterla al grafo.

Uso:
    python3 plot_odom.py /ruta/a/odom_deltas.csv      # → odom_trajectory.png
"""
import sys
import numpy as np
import pandas as pd
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python3 plot_odom.py /ruta/a/odom_deltas.csv")
        sys.exit(1)
