"""Ajusta el modelo de ruido del ArUco `std(d) = a + b·d` desde un CSV de detecciones.

Corre una vez (fuera de ROS) sobre el CSV del bag corto `aruco_estimation`, donde
alguien sostiene el marcador a distintas distancias, y produce `noise_model.json`
(consumido por `aruco_noise_model.NoiseModel`) más un gráfico de diagnóstico.

Método: el marcador NUNCA se sostuvo quieto en paradas limpias (se movió de forma
continua), así que no se puede agrupar por "distancias fijas". En cambio se usan
ventanas deslizantes cortas: dentro de cada ventana el movimiento es casi una recta,
así que se le quita la tendencia lineal (detrend) y el ruido se estima como la
desviación estándar de lo que sobra (los residuos). Luego se ajusta `std` vs.
distancia por mínimos cuadrados, por eje. Es una aproximación (no hubo paradas
limpias), suficiente para ponderar los edges del grafo.
...
def windowed_noise(df, window_n=20, step_n=10):
    """Estima el ruido local por ventanas deslizantes con detrending.

    Para cada ventana de `window_n` detecciones (avanzando `step_n` por paso):
    ajusta una recta a la serie temporal de cada eje (tx, ty, tz), se la resta
    (detrend) y toma la std de los residuos como ruido local. Devuelve un DataFrame
    con la distancia media de la ventana y la std por eje — la nube de puntos que
    luego ajusta `fit_linear` a `std(d) = a + b·d`.
    """
    rows = []
    n = len(df)
