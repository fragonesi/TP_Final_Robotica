"""Detector de marcadores ArUco — front-end de percepción de la Parte A.

Se suscribe al stream de la cámara del TurtleBot4, detecta marcadores ArUco,
estima la pose 3D de cada uno con solvePnP y registra todo a CSV (consumido luego
por GraphSLAM como observaciones rango-bearing). Cada ArUco actúa como un landmark
del SLAM, y su ID da la asociación de datos "gratis" (clave para el cierre de lazo).

Tres detalles NO obvios, cada uno capaz de tirar abajo todo el pipeline:

  1. **Diccionario**: el marcador real es `DICT_4X4_50`. El default de este nodo es
     `DICT_5X5_250`, que detecta *candidatos* (cuadrados) en cada frame pero decodifica
     CERO IDs. SIEMPRE pasar `-p aruco_dictionary:=DICT_4X4_50`. (`aruco_dict_probe.py`
     fuerza-bruta ~20 diccionarios contra el stream para confirmar cuál es.)

  2. **Resolución**: el único tópico de imagen del bag es el `preview` de baja
     resolución. El nodo lo agranda 4× (Lanczos, que conserva mejor el detalle que
     un resize común) ANTES de detectar, y escala la matriz K por el mismo factor
     (`_update_scaled_K`). Si `upscale_factor` no coincide entre el resize y el
     escalado de K, las distancias de solvePnP salen mal.

  3. **API de OpenCV**: soporta tanto la API nueva (>=4.7, `cv2.aruco.ArucoDetector`)
     como la vieja (<=4.6, `cv2.aruco.detectMarkers`); ROS Humble trae OpenCV 4.5.

Salidas:
  - CSV `log_csv_path`: una fila por detección (timestamp, marker_id, tx/ty/tz en el
    frame de la cámara, distancia, rvec).
  - `/landmarks_camera_frame` (PoseArray): poses en el frame ÓPTICO de la cámara
    (intermedio de depuración). NO es el `/landmarks` en frame del mapa que publica
    GraphSLAM; la conversión cámara→base y la corrección de pose ocurren más adelante.

Uso (¡con el override del diccionario!):
...
class ArucoDetectorNode(Node):
    def __init__(self):
        ...
        # Compatibilidad OpenCV: API nueva (>=4.7, ArucoDetector) vs vieja (<=4.6).
        self._use_new_aruco_api = hasattr(cv2.aruco, 'ArucoDetector')
        if hasattr(cv2.aruco, 'DetectorParameters'):
            self.aruco_params = cv2.aruco.DetectorParameters()
        else:
            self.aruco_params = cv2.aruco.DetectorParameters_create()
        # Parámetros afinados para una cámara real con marcadores chicos/lejanos y
        # motion blur (problemas que la consigna pide manejar explícitamente):
        #   minMarkerPerimeterRate=0.01  → acepta marcadores que ocupan muy pocos
        #     píxeles (lejos / preview de baja resolución).
        #   adaptiveThresh*            → barrido amplio de ventanas de umbralizado
        #     adaptativo, robusto ante iluminación despareja.
        #   CORNER_REFINE_SUBPIX       → refina esquinas a nivel subpíxel ⇒ solvePnP
        #     más preciso (la pose es muy sensible a la posición de las esquinas).
        #   errorCorrectionRate=0.8    → tolera más bits dañados al decodificar el ID
        #     (ayuda con frames borrosos sin disparar falsos positivos).
        ...
        if self._use_new_aruco_api:
            ...
            self.detector = None  # API vieja: se usa cv2.aruco.detectMarkers()
    ...
    def image_cb(self, msg: Image):
        ...
        if ids is not None and len(ids) > 0:
            # Estimación de pose por marcador con solvePnP. El marcador es un cuadrado
            # planar de lado `marker_length`; definimos sus 4 esquinas en el frame del
            # marcador (Z=0, centrado en el origen) en el MISMO orden que devuelve el
            # detector (top-left, top-right, bottom-right, bottom-left). Las unidades
            # de obj_points (metros) fijan la escala ⇒ marker_length debe ser el lado
            # real del tag, o las distancias salen con error de escala.
            ...
            for i, marker_id in enumerate(ids.flatten()):
                ...
                # IPPE_SQUARE: solver específico para marcadores cuadrados planos;
                # más estable y rápido que el PnP genérico para este caso.
