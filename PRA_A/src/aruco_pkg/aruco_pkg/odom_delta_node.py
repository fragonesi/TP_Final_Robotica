"""Odometría → deltas de movimiento (modelo de Thrun) para GraphSLAM.

Se suscribe a `/tb4_0/odom` y, entre cada par de poses consecutivas, calcula el
triplete `(δrot1, δtrans, δrot2)` del *odometry motion model* (Probabilistic
Robotics, cap. 5): girar δrot1, avanzar δtrans, girar δrot2. Estos deltas son las
restricciones de movimiento entre poses del grafo (los consume `graph_slam.py`).

Guard numérico: cuando δtrans es casi nulo (giro in-place o robot quieto),
`atan2(dy,dx)` es ruido puro, así que δrot1 se fuerza a 0 y toda la rotación se
carga en δrot2. Sin este guard se inyecta ruido artificial al grafo. (Mismo
criterio que tp2/tp5 de la cursada.)

Salidas: CSV `log_csv_path` (timestamp, x, y, theta, δrot1, δtrans, δrot2, dt) y
`/odom_deltas` (Vector3 = rot1, trans, rot2).

GOTCHA de QoS: el bag publica `/tb4_0/odom` como BEST_EFFORT; la suscripción debe
declararlo explícitamente o ROS 2 (default RELIABLE) no entrega NINGÚN mensaje.
