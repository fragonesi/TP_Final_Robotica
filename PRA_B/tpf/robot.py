    ...
    def _build_planning_map(self) -> OccupancyGrid:
        ...
        for rng in scan.ranges:
            ...
            if rng < scan.range_min or rng > scan.range_max:
                continue
            # Sólo nos importan obstáculos razonablemente cerca (opcional) MAX_DYNAMIC_RANGE hyperparam hiperparam ***
            if rng > 3.0:
                continue
        ...
        n = int((dilated & (np.array(planning.data).reshape(height, width) == 100)).sum())
        self.get_logger().info(
